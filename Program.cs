    ...
    class Program
    {
        static async System.Threading.Tasks.Task Main(string[] args)
        {
            ...
            while (matchid == 0)
            {
                ...
                bool success = int.TryParse(Console.ReadLine(), out matchid);
                if (!success)
                {
            ...
            foreach (var game in json["games"])
            {
                ...
                foreach (var score in game["scores"])
                {
                    ...
                    newScore.userid = int.Parse(score["user_id"].ToString());

                    String userResponse = await client.GetStringAsync(client.BaseAddress + "get_user?k=" + apikey + "&u=" + newScore.userid);
            ...
            StreamWriter file = new(lobbyname + ".csv", append: true);
